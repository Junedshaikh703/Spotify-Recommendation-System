...
class HybridRecommenderSystem:
    ...
    def calculate_collaborative_filtering_similarities(self, song_name, artist_name, songs_data, interaction_matrix):
        # fetch the row from songs data
        song_row = songs_data.loc[(songs_data["name"] == song_name) & (songs_data["artist"] == artist_name)]
        
        # dataframe index = interaction matrix row
        ind = song_row.index[0]
    ...
    def give_recommendations(self):
        ...
        # get top k songs directly using aligned indices
        top_k_songs = (
                        self.songs_data
                        .iloc[recommendation_indices]
...
if __name__ == "__main__":
    ...
    transformed_data = load_npz("data/transformed/transformed_hybrid_data.npz")
    
    # load the interaction matrix
    interaction_matrix = load_npz("data/transformed/interaction_matrix.npz")
    
    # load the songs data
    songs_data = pd.read_csv("data/cleaned/collab_filtered_data.csv",
